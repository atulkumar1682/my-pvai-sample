#!/bin/sh
#__________ S3 Backup for Atlassian Application _____________________________________
# Version: 1.2.2 November 22 2019

#__________ Application Directories _________________________________________________
...
DATABASE=`/bin/cat ${HOME}/.pgpass | grep "jira_qa" | sed  "s/:.*$//"`

# Set Local Backup Directory
BAD='/opt/atlassian/application-data/jira/data'
BK='/efs/atlassian/jira_qa/backup'

# Command
COMMAND="Usage: $0 [ Retention day(s) ]"

#__________ Set retention Period at 1st argument. _________________________________
if [ $# -ne 1 ]; then
    echo $COMMAND;
    exit 0;
fi
...
#_________ Check this application is still running._______________________________
X=`pgrep -f ${AP}/conf`
...
#__________ Get today's date. ____________________________________________________
TODAY=`/bin/date "+%Y%m%d"`

#__________ Backup up to Local directory _________________________________________
if [ ${#BAD} -ne 0 ]; then
   LB=`/usr/bin/zip -r ${BK}/${TODAY}.zip ${BAD}`
fi

#__________ Get delete date based on the retention day(s) ________________________
...
#__________ Get number of backup files in S3 ___________________________________
C=`aws s3 ls ${F}/`
...
if [ ${NEWEST} -lt ${TODAY} ]; then
        ...
        /usr/bin/pg_dump -w -h $DATABASE -U pvai -c --if-exists -d jira_qa | gzip > $HOME/atlassian/Backup/jira_qa.sql.gz

        if [ -e $HOME/atlassian/Backup/jira_qa.sql.gz ]; then
           /usr/bin/aws s3 cp $HOME/atlassian/Backup/jira_qa.sql.gz  $F/${TODAY}/jira_qa.sql.gz --quiet
        fi
...
if [ ! -z $LP ] ; then
   ...
   if [ -e $HOME/atlassian/Backup/jira_qa.sql.gz ]; then
      R=`aws s3 cp $HOME/atlassian/Backup/jira_qa.sql.gz  $F/latest/jira_qa.sql.gz --quiet`
   fi
