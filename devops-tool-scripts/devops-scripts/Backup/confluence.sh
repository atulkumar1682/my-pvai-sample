...
#__________ S3 Backup for Atlassian Application _______________________________
# Version: 1.2.1 November 22 2019

#__________ Application Directories ___________________________________________
...
DATABASE=`/bin/cat ${HOME}/.pgpass | grep "confluence" | sed  "s/:.*$//"`


#__________ Command __________________________________________________________
...
if [ ${NEWEST} -lt ${TODAY} ]; then
        ...
        /usr/bin/pg_dump -w -h $DATABASE -U pvai -c --if-exists -d confluence | gzip > $HOME/atlassian/Backup/confluence.sql.gz

        if [ -e $HOME/atlassian/Backup/confluence.sql.gz ]; then
           /usr/bin/aws s3 cp $HOME/atlassian/Backup/confluence.sql.gz  $F/${TODAY}/confluence.sql.gz --quiet
        fi
...
if [ ! -z $LP ] ; then
   ...
   if [ -e $HOME/atlassian/Backup/confluence.sql.gz ]; then
      R=`aws s3 cp $HOME/atlassian/Backup/confluence.sql.gz  $F/latest/confluence.sql.gz --quiet`
   fi
