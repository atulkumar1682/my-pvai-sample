...
if [ ${S3DB} = "Y" ] || [ ${S3DB} = "y" ]; then
    DB=`aws s3 cp $F/${RET}/${USER}_sd.sql.gz ${HOME}/.`

    if [ -e ${HOME}/${USER}_sd.sql.gz ]; then
       /bin/gzip -cd ${HOME}/${USER}_sd.sql.gz | /usr/bin/psql -w -h $DATABASE -U pvai -d ${USER}_sd
       /bin/rm ${HOME}/${USER}_sd.sql.gz
    fi
fi
...
<jira-database-config>
  ...
  <jdbc-datasource>
    <url>jdbc:postgresql://$DATABASE:5432/jira_sd</url>
...
echo "JIRA Service Desk will start after 10 seconds. If you don't want to, please type cnt+C now."
sleep 10
